...
void Atry1Hud::DrawHUD()
{
	...
	if( bHMDDeviceActive == false )
	{
		...
		Atry1Pawn* Vehicle = Cast<Atry1Pawn>(GetOwningPawn());
		if ((Vehicle != nullptr) && (Vehicle->bInCarCameraActive == false))
		{
			...
			// Error Speed
			FCanvasTextItem ThrottleTextItem(FVector2D(HUDXRatio * 805.f, HUDYRatio * 600), Vehicle->ThrottleDisplayString, HUDFont, FLinearColor::Red);
